...
def load_stage0_frozen_params(model, checkpoint_path):
    """
    ...
    # 1. Load Speech Encoder
    speech_encoder_dict = {k.replace('module.speech_encoder.', ''): v
                            for k, v in state_dict.items()
                            if k.startswith('module.speech_encoder.')}
    if speech_encoder_dict:
        msg = model.speech_encoder.load_state_dict(speech_encoder_dict, strict=True)
        print(f"Successfully loaded Speech Encoder: {msg}")
    else:
        print("Warning: No Speech Encoder weights found in checkpoint.")
...
def trainer_synapse(model):
    ...
    data_path = r"E:\dataset\ultrasound_video_audio\DATA\dataset\train_dataset.csv" #正样本
    ...
    #finetune
    # fitune_model_path = r"Z:\dataset\accelerometer\record\RESULT_original_removeTimbre_average_swinUnet_nonCrossUser_simulation\best_model.pth"
    # model.load_state_dict(torch.load(fitune_model_path))
    ...
    for epoch in iterator:
        ...
        if (epoch + 1) % 10 == 0:
            ...
            torch.save(model.state_dict(), f'./checkpoints/stage1/stage1_model_epoch_{epoch+1}.pth')
            print(f'Model saved at epoch {epoch+1}')

        if epoch == 0 or loss.item() < best_loss: #训练集上损失最小，不代表在测试集上效果最好。一般取的是验证集准确度最高或者验证集损失最小进行存储
            best_loss = loss.item()
            torch.save(model.state_dict(), './checkpoints/stage1/stage1_best_model.pth') #多卡训练时，应该改成 torch.save(model.module.state_dict(), './outputs/best_model.pth')
    ...
    with open('./outputs/stage1/stage1_train_process.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
