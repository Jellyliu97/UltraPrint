...
from model.stage2_model2 import Stage2Model
...
def load_stage1_1_frozen_params(model, checkpoint_path):
    """
...
def trainer_synapse(model):
    ...
    # train_loader = DataLoader(full_dataset, batch_size=batch_size, sampler=sampler, shuffle=False, num_workers=8, pin_memory=True, persistent_workers=True) #每个epoch动态采样负样本
    ...
    pretrain_model_path = r"./checkpoints/stage1_1/stage1_1_best_model.pth"
    load_stage1_1_frozen_params(model, pretrain_model_path)
    
    # freeze_modules = [model.acc_encoder, model.fusion_network]
    ...
    for epoch in iterator:
        ...
        #     epoch, batch_loss))
        # print(f'Loss1: {(1-loss1).mean().item():.4f}, Loss2: {loss2.item():.4f}')
