...
class Stage2Model(nn.Module):
    ...
    def forward(self, waveform, image_features):
        ...
        # recon_v = self.model_u2v(u)  # Reconstructed video features from us 
        # recon_u = self.model_v2u(v)  # Reconstructed us features from video
        # diff_u = u - recon_u
        # diff_v = v - recon_v
        # pooled_diff_u = torch.mean(diff_u, dim=1)
        # pooled_diff_v = torch.mean(diff_v, dim=1)  
        ...
        # class_features = pooled_features + pooled_diff_u #  + pooled_diff_v
        class_features = pooled_features
