...
class WaveformModel(nn.Module):
    ...
    def forward(self, x, y):
        # x: (B, 1, 3000*T)
        # x: us , y: video features (B, 10*T, feature_dim)
        # Encode -> Intermediate feature [Batch, 10*T, D]
        latent_features = self.encoder(x)
        
        recon_v = self.model_u2v(latent_features)  # Reconstructed video features from us 
        recon_u = self.model_v2u(y)  # Reconstructed us features from video
        
        return latent_features, recon_v, recon_u
