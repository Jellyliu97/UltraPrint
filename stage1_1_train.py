...
from model.stage1_1_mode1 import WaveformModel
...
def trainer_synapse(model):
    ...
    for epoch in iterator:
        ...
        for i_batch, batch in tqdm(enumerate(train_loader), desc=f"Train: {epoch}", total=len(train_loader),
            ...
            loss1 = critertion_MSE1(recon_u, feature_u)  #重建us损失
            loss2 = critertion_MSE2(recon_v, video_feature)  #重建video损失
        ...
        #     epoch, batch_loss))
        print(f'mse Loss1: {loss1.item():.4f}, mse Loss2: {loss2.item():.4f}')
        print(f'Epoch {epoch+1}/{num_epochs}, Loss: {loss.item()}')
        all_losses.append([epoch+1, loss.item()])     

        if (epoch + 1) % 10 == 0:
            scheduler.step() #动态更新学习率,每10个epoch更新一次学习率
            torch.save(model.state_dict(), f'./checkpoints/stage1_1/stage1_1_model_epoch_{epoch+1}.pth')
            print(f'Model saved at epoch {epoch+1}')

        if epoch == 0 or loss.item() < best_loss: #训练集上损失最小，不代表在测试集上效果最好。一般取的是验证集准确度最高或者验证集损失最小进行存储
            best_loss = loss.item()
            torch.save(model.state_dict(), './checkpoints/stage1_1/stage1_1_best_model.pth') #多卡训练时，应该改成 torch.save(model.module.state_dict(), './outputs/best_model.pth')
    ...
    with open('./outputs/stage1_1/stage1_1_train_process.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
