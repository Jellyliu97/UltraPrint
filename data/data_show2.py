...
def load_and_sample_data(root_path, samples_per_user=50):
    """
    ...
    # Ensure we only pick 10 random users if there are more than 10
    # if len(user_dirs) > 20:
        # user_dirs = np.random.choice(user_dirs, 10, replace=False)
    
    print(f"Found {len(user_dirs)} users.")

    for user_id, user_name in enumerate(user_dirs):
        user_path = os.path.join(root_path, user_name)
        # npy_files = glob.glob(os.path.join(user_path, "*_meanVideoFeature.npy"))
        # npy_files = glob.glob(os.path.join(user_path, "*_midPool.npy"))
        
        # npy_files = glob.glob(os.path.join(user_path, "*_v.npy"))
        npy_files = glob.glob(os.path.join(user_path, "*_u.npy"))
        # npy_files = glob.glob(os.path.join(user_path, "*_fusion.npy"))
        ...
        if num_available >= samples_per_user:
            ...
            print(f"Warning: User {user_name} has only {num_available} samples (requested {samples_per_user}). Skipping.")
            continue
            
        print("shape of sampled_data=======:", sampled_data.shape)
...
def plot_tsne(X, y, user_names):
    save_dir="/root/UltraPrint/data/show_results"
    """
...
if __name__ == "__main__":
    ...
    # DATA_ROOT = "/root/autodl-tmp/Results/stage1_test_dataset" #stage1 
    
    # DATA_ROOT = "/root/autodl-tmp/Results/test_dataset" 
    # DATA_ROOT = "/root/autodl-tmp/Results/test_dataset_fakeMismatch"
    DATA_ROOT = "/root/autodl-tmp/Results/test_dataset_crossUser" 
    ...
    if not os.path.exists(DATA_ROOT):
        ...
        try:
            X, y, users = load_and_sample_data(DATA_ROOT, samples_per_user=3)
            plot_tsne(X, y, users)
        except Exception as e:
